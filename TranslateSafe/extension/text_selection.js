  ...
  debounceTimer = setTimeout(async () => {
    ...
    if (text && text.length > 0) {
      ...
      try {
        ...
        console.warn("TranslateSafe: Context invalidated or error reading storage.", err);
      }
    }
...
function updateBubble(translatedText, originalText, targetLang, isError = false) {
  ...
  if (!isError && originalText) {
    ...
    currentBubble.appendChild(btn);
      ...
      hint.innerHTML = "Open <a href='http://localhost:3000' target='_blank' style='color:#818cf8'>TranslateSafe App</a> to sync key.";
